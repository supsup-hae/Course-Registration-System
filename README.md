...
### 1. 강의관리
 ...
 - **강의 상태 전환**
   - 허용되는 전환: `DRAFT → OPEN`, `OPEN → CLOSED`, `CLOSED → OPEN` (재오픈)
   - 금지되는 전환: 모든 상태에서 `DRAFT`로의 복귀, `DRAFT → CLOSED` 직행
   - `OPEN` 전환 시 `startDate` 필수, `endDate`는 `null` 허용 (무기한 종료)
   - `startDate`와 `endDate` 모두 지정한 경우 `endDate > startDate` 검증

 - **강의 조회 (목록/상세)**
   - **조회 성능 최적화 (Redis Caching)**: 반복적인 DB I/O를 줄이고 응답성을 극대화하기 위해 강의 상세 내역에 Redis 캐싱 적용. 직렬화 시 `GzipRedisSerializer`를 도입해 캐시 메모리 및 네트워크 대역폭 최적화
   - **목록 페이징 및 다중 필터링**: 목록 페이징(Pagination) 지원 및 강의 상태, 가격 범위, 빈자리 여부(hasCapacity) 기반 검색 필터 적용

### 2. 수강 신청

 - **동시성 정원 초과 방어**
   - **Redis Counter**를 활용하여 인메모리 상에서 1차로 수강 슬롯을 빠르게 선점 및 검증
   - DB 저장 단계에서 **비관적 락(Pessimistic Lock)**을 병행 검증하여 동시성 이슈로 인한 정원 초과 승인을 차단
   
 - **수강 신청 만료 스케줄러 및 보정 정책**
   - `PENDING` 상태(임시 신청) 건은 제한 시간(15분) 내 결제 확정(`CONFIRMED`) 미완료 시 타임아웃 대상으로 분류
   - 스케줄러를 통해 만료 건을 `CANCELLED` 상태로 일괄 변경하고, 선점된 수강 정원(Redis Counter)을 잔여 슬롯으로 롤백(보정) 처리
