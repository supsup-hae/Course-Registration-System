...
BEGIN
    ...
    IF NOT EXISTS (SELECT 1 FROM pg_constraint WHERE conname = 'ck_courses_capacity') THEN
        ALTER TABLE courses ADD CONSTRAINT CK_COURSES_CAPACITY CHECK (capacity > 0);
    END IF;
...
CREATE UNIQUE INDEX UX_ENROLLMENTS_ACTIVE_STUDENT_COURSE ON enrollments (student_id, course_id) WHERE status IN ('PENDING', 'CONFIRMED', 'WAITLISTED');
